...
static auth_acl_t authenticateAuthenticate(auth_user_request_t ** auth_user_request, http_hdr_type headertype, request_t * request, ConnStateData * conn, sqaddr_t *src_addr);
...
static void
authenticateAuthUserRequestLinkIp(auth_user_request_t * auth_user_request, sqaddr_t *ipaddr, request_t * request)
{
    auth_user_request_ip_hash_t *hash_entry;
...
static void
authenticateAuthUserRequestSetIp(auth_user_request_t * auth_user_request, sqaddr_t *ipaddr, request_t * request)
{
...
void
authenticateAuthUserRequestRemoveIp(auth_user_request_t * auth_user_request, sqaddr_t *ipaddr)
{
 ...
 */
auth_acl_t
authenticateAuthenticate(auth_user_request_t ** auth_user_request, http_hdr_type headertype, request_t * request, ConnStateData * conn, sqaddr_t *src_addr)
{
	...
	if (proxy_auth && !request->auth_user_request && conn && conn->auth_user_request) {
		...
		debug(29, 1) ("authenticateAuthenticate: Unexpected change of authentication scheme from '%s' to '%s' (client %s)\n",
		    authscheme_list[conn->auth_user_request->auth_user->auth_module - 1].typestr, proxy_auth, cbuf);
		authenticateAuthUserRequestUnlock(conn->auth_user_request);
		conn->auth_user_request = NULL;
		conn->auth_type = AUTH_UNKNOWN;
	    }
...
auth_acl_t
authenticateTryToAuthenticateAndSetAuthUser(auth_user_request_t ** auth_user_request, http_hdr_type headertype, request_t * request, ConnStateData * conn, sqaddr_t *src_addr)
{
