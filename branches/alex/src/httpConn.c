...
httpConnClosed(int fd, void *data)
{
    ...
    assert(conn != NULL);
    while ((http = conn->chr) != NULL) {
	assert(http->conn == conn);
	assert(conn->chr != conn->chr->next);
	httpXactionFree(http);
    }
    if (conn->ident.fd > -1)
	comm_close(conn->ident.fd);
    safe_free(conn->in.buf);
    meta_data.misc -= conn->in.size;
    pconnHistCount(0, conn->nrequests);
    cbdataFree(conn);
}
...
/*
 * reads Res (REquests or REplies) from socket; uses configured functions to
 * generate Res and test their ability to accept data. It is Res responsibility
 * to register their current state with the connection using well-known
 * functions.
 */
static ???
httpConnReadRes(HttpConn *conn)
{
    int fd;
    ...
    debug(12, 4) ("httpConnReadRes: FD %d: reading...\n", fd);

    /* always maintain registered status */
    commSetSelect(fd, COMM_SELECT_READ, conn->reader, conn, 0);
    if (conn->in_buf->free_space > 0) {
    	size = ioBufferRead(conn->in_buf, fd, conn);
    	...
    	if (size == 0) {
    	    ...
    	    debug(12, 5) ("httpConnReadRes: FD %d closed?\n", fd);
    	    /* EBIT_SET(F->flags, FD_SOCKET_EOF); @?@ not used anyway */
    	    /* defer next check for a while of nothing is waiting */
    	...
    	if (size < 0) {
    	    ...
    	    if (!ignoreErrno(errno)) {
    	    	debug(12, 2) ("httpConnReadRes: FD %d: %s\n", fd, xstrerror());
    	    	comm_close(fd);
    	    	return; /* must exit after close */
    	    }
    	    return; /* exit, we did not change anything */
    ...
    if (conn->read_queue->head && conn->in_buf->size)
    	conn->read_queue->head->noteDataReady(); /* it has a pointer to in_buf */
    /*
     * create res while (a) we have data in the buffer, (b) buffer is not
     * locked, (c) admission policy OKs
     */
    while (conn->in_buf->size && !conn->in_buf->read_lock && httpConnCanAdmit(conn)) {
    	conn->startReader(conn);
    }
}
...
/*
 * @?@ Check this: We want to say more than just a general timeout message. If
 * there is a pending re, let it handle the situation.  If no res are pending,
 * we have to deal with situation ourselves. This should work for both active
...
{
    HttpConn *conn = data;
    debug(12, 2) ("httpConnTimeout: FD %d: lifetime is expired.\n", fd);
BAD:

(good for passive, bad for active)
    if (conn->write_queue->head) /* writer might know more, put it first? */
	httpConnReportTimeout(conn, conn->write_queue->head);
    else
    if (conn->read_queue->head) /* reader may know something */
	httpConnReportTimeout(conn, conn->read_queue->head);
    else
    if (!fd_table[fd].rwstate) /* nobody, and no prior activity @?@ */
	httpConnReportTimeout(conn, 0);
    else {
	comm_close(fd); /* idle persistent connection? */
	return;
    }
    /*
...
static void
httpConnReportTimeout(conn, HttpRe *info_source);

    ErrorState *err = errorCon(ERR_LIFETIME_EXP, HTTP_REQUEST_TIMEOUT);
    err->url = xstrdup("N/A");
	/*
	 * Normally we shouldn't call errorSend() in httpConn.c, but
 ...
 *	- accept()
 *	- allocate conn and init its members 
 *      - register connFree as conn's destructor on connection close
...
httpAccept(int sock, void *notused)
{
    ...
    commSetDefer(fd, clientReadDefer, conn);
}
...
static clientHttpRequest *parseHttpRequest(HttpConn *, method_t *, int *, char **, size_t *);
...
parseHttpRequest(HttpConn * conn, method_t * method_p, int *status,
