...
fwdConnectStart(void *data)
{
    ...
    sqinet_set_v4_inaddr(&fwdState->request->out_ip, &outgoing);
...
fwdDispatch(FwdState * fwdState)
{
    ...
    sqinet_copy(&fwdState->request->out_ip, &fd_table[server_fd].local_address);
...
fwdStart(int fd, StoreEntry * e, request_t * r)
{
     ...
     */
    if (! sqinet_is_noaddr(&r->client_addr) && r->protocol != PROTO_INTERNAL && r->protocol != PROTO_CACHEOBJ) {
	/*      
